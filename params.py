...
NUM_RL_AGENTS = len(AGENT_IDS)          # Number of agents that are trained. The others are dummy agents (always choosing power rate 1.0)
...
TRAINING_PERIOD = TRAINING_END_DAY - TRAINING_START_DAY     # The maximum length of the training period
TESTING_PERIOD = TESTING_END_DAY - TESTING_START_DAY     # The maximum length of the training period
...
REPLACE_TARGET_INTERVAL = 50     # After so many episodes the target network is replaced with hard update
...
MAX_INCENTIVE = 10
DISCOUNT_RATE_AGGREGATOR = 0.9  # Discount rate (Gamma) for the aggregator
...
DISSATISFACTION_COEFFICIENTS_NEW = np.array([8.0, 0.01, 0.4, 0.1, 0.2])                  # Delay coefficients for households test
#DISSATISFACTION_COEFFICIENTS = np.array([0.5, 0.05, 0.2, 0.1, 0.4])                  # Delay coefficients
...
# Simplest version
PRICES = np.array(
    [21, 21, 21, 21, 20, 20, 20, 20, 19, 19, 19, 19, 18, 18, 18, 18, 18, 18, 18, 18, 19, 19, 19, 19, 24, 24, 24, 24, 28, 28, 28, 28, 29, 29, 29, 29, 30, 30, 30, 30, 31, 31, 31, 31, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 33, 33, 33, 33, 35, 35, 35, 35, 37, 37, 37, 37, 36, 36, 36, 36, 35, 35, 35, 35, 34, 34, 34, 34, 31, 31, 31, 31, 26, 26, 26, 26, 23, 23, 23, 23])